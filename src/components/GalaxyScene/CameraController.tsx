...
const CameraController: React.FC<CameraControllerProps> = ({ target, zoom = 1, sceneCenter = null, awaken = true, scale = 1, centerOnTarget = false }) => {
  ...
  useFrame((_, delta) => {
    // compute desired camera destination
    if (target) {
      ...
      if (!centerOnTarget && sceneCenter) {
        ...
        // move target ~28% toward center for better framing of neighbors
  ...
  const startOffset = new Vector3(0, 1.4, 20); // slightly farther start for dramatic reveal
  const desired = new Vector3(tgt.x, tgt.y, tgt.z + dist);
      ...
      // use slightly stronger lerp when centering to make motion feel snappier
